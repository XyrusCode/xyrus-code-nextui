...
export const NowPlayingIframe = ({ songId }: { songId: string }) => {
	return (
		<div className="w-full h-16 sm:h-20">
			<iframe
				...
				width="100%"
				height="100"
				allow='encrypted-media; fullscreen; picture-in-picture; autoplay; clipboard-write;'
				allowTransparency
				loading='lazy'
			/>
		</div>
...
const NowPlaying = () => {
	...
	return (
		...
		<div className="flex flex-row-reverse items-center sm:flex-row mb-8 space-x-0 sm:space-x-2 w-full">
			{data?.songUrl ? (
				<AnimatedBars />
				// <NowPlayingIframe songId={data?.id} />
			) : (
				<svg className="h-4 w-4 ml-auto mt-[-10px]" viewBox="0 0 168 168">
					<path
						fill="#1ED760"
